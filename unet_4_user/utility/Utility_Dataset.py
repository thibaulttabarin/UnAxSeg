...
def get_data(path=None):
    '''
    Loads images and the mask in the same order from /image*.png and /mask*.png into a numpy array
    input : path of the folder containing /image*.png and /mask*.png (usually train or validation)
    output : list of tuple (image as nparray, mask as nparray)
    '''
    ...
    if path == None or not os.path.isdir(path):
            ...
            path =  filedialog.askdirectory(initialdir = './',\
                                                      title = 'choose directory containing "dataset (image*.png/mask*.png)"') 
        
    
    img_paths = natsorted(glob.glob(path + "/image*.png"))
    mask_paths = natsorted(glob.glob(path + "/mask*.png"))
    
    X_data = np.array([skimage.io.imread(path)for path in img_paths], dtype=np.uint8)  
    Y_data = np.array([skimage.io.imread(path) for path in mask_paths], dtype=np.uint8)
        
    Data = list(zip(X_data ,Y_data))
    
    return Data

##TODO : get_data_2 should be made default or remove
def get_data_2(path=None):
    '''
    Loads images and the mask in the same order from /image*.png and /mask*.png into a numpy array
    Using PIL module instead of skimage.
    input : path of the folder containing /image*.png and /mask*.png (usually train or validation)
    output : list of tuple (image as nparray, mask as nparray)
    '''
    ...
    X_data = np.array([np.array(Image.open(path))for path in img_paths], dtype=np.uint8)  
    Y_data = np.array([np.array(Image.open(path)) for path in mask_paths], dtype=np.uint8)
