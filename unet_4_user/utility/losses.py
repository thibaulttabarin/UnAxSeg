...
def hard_dice_coef_ch0(y_true, y_pred, smooth=1e-3):
...
def dice_coef_ch1(y_true, y_pred, smooth=1e-3):
    y_true_f = K.flatten(y_true[..., 1])
    y_pred_f = K.flatten(y_pred[..., 1])
    intersection = K.sum(y_true_f * y_pred_f)
    return K.mean((2. * intersection + smooth) / (K.sum(y_true_f) + K.sum(y_pred_f) + smooth))

def dice_coef_ch2(y_true, y_pred, smooth=1e-3):
...
def double_head_loss(y_true, y_pred):
    mask_loss = dice_coef_loss_bce(y_true[..., 1], y_pred[..., 1])
    contour_loss = dice_coef_loss_bce(y_true[..., 2], y_pred[..., 2])
...
def mask_contour_mask_loss(y_true, y_pred):
    ...
    loss = mask_loss + 2 * contour_loss + full_mask
    return loss

def softmax_dice_loss(y_true, y_pred):
    loss = categorical_crossentropy(y_true, y_pred) * 0.6 + \
        dice_coef_loss(y_true[..., 1], y_pred[..., 1]) * 0.2 + \
        dice_coef_loss(y_true[..., 2], y_pred[..., 2]) * 0.2

    return loss

def softmax_dice_loss_2(y_true, y_pred):
...
def softmax_dice_loss_1(y_true, y_pred):
    
    loss = 0.4* binary_crossentropy(y_true, y_pred) + \
        0.6* dice_coef_loss(y_true, y_pred)
    return loss
