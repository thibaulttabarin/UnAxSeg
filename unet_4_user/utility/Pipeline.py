...
class DataPipeline(Pipeline):
    ...
    def keras_generator_with_mask(self, batch_size=1, image_data_format="channels_last"):
        
         
        '''
        Add by Thibault Tabarin
            Returns an image generator that will sample from the current Datapipeline
            indefinitely, as long as it is called.
            It takes into account mask and transform data to keras format (batch_size, width, height, channels)
            this is a test version 
        '''
        # If the number is 0 or negative, default it to 1
        batch_size = 1 if (batch_size < 1) else batch_size
         
        def keras_format(PIL_Image):
            
             
            np_array = np.asarray(PIL_Image)
             
            if np.ndim(np_array) == 2:   
        ...
        while True:
            ...
            for i in range(0, batch_size):
                ...
                images_to_yield = [keras_format(x) for x in images_to_yield]

                batch_x.append(images_to_yield[0])
                batch_y.append(images_to_yield[1])
                
            images = np.asarray(batch_x)
            masks =np.asarray(batch_y)
            yield (images, masks)         
